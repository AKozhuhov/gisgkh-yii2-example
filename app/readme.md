...
Модуль подключен через менеджер зависимостей composer. Настрйки подключения в файле [`composer.json`](composer.json):
...
Подключение в приложении и параметры настройки определеный в файлах кофигурации 
`config/console.php` (для консольных команд) и `config/web.php` (для веб-приложения):
